...
def get_with(tag_id, organization_id):
    ...
    if not tag:
        raise Exceptions.NotFound(
            f"The tag with id {tag_id} and organization {organization_id} was not found"
            )
    return tag
...
def tag_asset(tag_id, asset_id, asset_type, organization_id):
    ...
    if not is_from_organization(tag_id, organization_id):
        raise Exceptions.Forbidden("Trying to use a tag from other organization.")
    if not DeviceRepository.is_from_organization(asset_id, organization_id):
        raise Exceptions.Forbidden("Trying to tag an asset from other organization")
    ...
    if asset_type=="device":
        ...
        raise Exceptions.BadRequest(f"Invalid asset_type: {asset_type}")
    db.session.commit()
...
def untag_asset(tag_id, asset_id, asset_type, organization_id):
    ...
    """
    if not is_from_organization(tag_id, organization_id):
        raise Exceptions.Forbidden("Trying to delete a tag from other organization.")
    if not DeviceRepository.is_from_organization(asset_id, organization_id):
        raise Exceptions.Forbidden("Trying to tag an asset from other organization")
