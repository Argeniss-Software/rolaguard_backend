...
from iot_api.user_api.repository import TagRepository, AssetRepository
...
class AssetImportanceAPI(Resource):
    ...
    def post(self):
        ...
        importance = request.args.get('importance', type=str, default=None)

        if importance not in ['LOW', 'MEDIUM', 'HIGH']:
            raise Exception(f'"{importance}" is not a valid importance value')

        for asset_ids in asset_list:
            asset_ids = json.loads(asset_ids.replace("\'", "\""))
